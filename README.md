...
> [!NOTE]
> Package endpoints keep detailed version data. Owner and repo aggregate endpoints adapt their version detail to a byte budget so large self-hosted indexes can still finish JSON/XML publication. `BKG_OWNER_ARRAY_MAX_BYTES` defaults to `35000000`; `BKG_OWNER_ARRAY_VERSION_LIMIT` can force a fixed per-package version limit, and negative values keep full version arrays. JSON/XML trimming uses `BKG_JSON_XML_MAX_BYTES` (`50000000`) and `BKG_JSON_XML_HARD_MAX_BYTES` (`100000000`).
